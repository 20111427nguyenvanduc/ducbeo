...
module.exports = function(req, res) {
  ...
  async.waterfall([
    function checkParams(cb) {
      ...
      if (!req.admin.comparePassword(currentPassword)) return cb({ code: 400, message: Message.PASSWORD_WRONG })
      if (newPassword.length < 6) return cb({ code: 400, message: { head: 'Lỗi', body: 'Mật khẩu mới phải có ít nhất 6 ký tự' } })
      cb(null)
    },
    function update(cb) {
      var passwordHash = AdminModel.hashPassword(newPassword)
      AdminModel.findByIdAndUpdate(req.admin._id, { passwordHash: passwordHash }, function(err) {
