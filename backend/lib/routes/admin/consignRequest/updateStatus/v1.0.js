...
module.exports = function(req, res) {
  ...
  async.waterfall([
    ...
    function update(cb) {
      ConsignRequestModel.findByIdAndUpdate(id, { status: status }, { new: true }, function(err, doc) {
        if (err) return cb({ code: 500, message: Message.SYSTEM_ERROR })
        if (!doc) return cb({ code: 404, message: Message.CONSIGN_REQUEST_NOT_FOUND })
        cb(null, doc)
      })
    },
  ], function(err, doc) {
    if (err) return res.json(err)
    res.json({ code: 200, message: Message.CONSIGN_REQUEST_UPDATED, data: doc })
  })
}
